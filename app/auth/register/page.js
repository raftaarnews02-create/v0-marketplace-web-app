...
import { Phone, ArrowRight, Loader2, ArrowLeft, Shield, Sparkles, User, Store } from 'lucide-react';
...
export default function Register() {
  ...
  const [formData, setFormData] = useState({ name: '', userType: 'buyer' });
  ...
  const verifyOTP = async (e) => {
    ...
    try {
      ...
      // If user already has a name, they're already registered → go home
      if (data.user?.name) {
  ...
  const completeProfile = async (e) => {
    ...
    try {
      const res = await fetch('/api/auth/register', {
        method: 'POST', headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ phone, name: formData.name, userType: formData.userType }),
      });
      const data = await res.json();
      if (!res.ok) { setError(data.error || 'Registration failed'); return; }
      login(data.token, data.user);
      router.push(formData.userType === 'vendor' ? '/sell' : '/');
  ...
  const stepLabels = { phone: '1', otp: '2', profile: '3' };
  const stepNames  = ['Phone', 'Verify', 'Profile'];
  ...
  return (
    <main className="min-h-screen flex flex-col" style={{ background: BG }}>
      ...
      <div className="max-w-md mx-auto w-full px-4 pt-5">
        <div className="flex items-center justify-center gap-2">
          {stepNames.map((name, i) => {
            const key = ['phone', 'otp', 'profile'][i];
            const isActive = step === key;
            const isDone = (step === 'otp' && i === 0) || (step === 'profile' && i <= 1);
            return (
              <React.Fragment key={name}>
                <div className="flex flex-col items-center gap-1">
                  <div className="w-7 h-7 rounded-full flex items-center justify-center text-xs font-bold"
                    style={{
                      background: isDone ? ACC : isActive ? '#3b82f6' : SURF2,
                      color: isDone ? '#0a1628' : '#fff',
                      border: isActive ? '2px solid #3b82f6' : '2px solid ' + BOR,
                    }}>
                    {isDone ? '✓' : i + 1}
                  </div>
                  <span className="text-[9px] font-semibold" style={{ color: isActive ? PRI2 : T3 }}>{name}</span>
                </div>
                {i < 2 && <div className="w-8 h-0.5 mb-4" style={{ background: isDone ? ACC : BOR }} />}
              </React.Fragment>
            );
          })}
        </div>
      ...
      <div className="flex-1 flex flex-col justify-center max-w-md mx-auto w-full px-4 py-6">
        ...
        <div className="rounded-3xl p-6 shadow-2xl" style={{ background: SURF, border: '1px solid ' + BOR }}>
          ...
          <h1 className="text-xl font-bold text-center mb-1" style={{ color: T1 }}>
            {step === 'phone' ? 'Create Account' : step === 'otp' ? 'Verify Number' : 'Your Profile'}
          </h1>
          <p className="text-sm text-center mb-6" style={{ color: T3 }}>
            {step === 'phone' ? 'Join Zubika — first 2 listings FREE' :
          ...
          {step === 'otp' && (
            <form onSubmit={verifyOTP} className="space-y-4">
              ...
              <div className="text-center">
                {timer > 0 ? (
                  <p className="text-xs" style={{ color: T3 }}>Resend in {timer}s</p>
                ) : (
            ...
            <form onSubmit={completeProfile} className="space-y-4">
              <div>
                ...
                <input type="text" value={formData.name}
                  ...
                  placeholder="Enter your full name" className={IC} style={IS} autoFocus />
              </div>

              <div>
                <label className="block text-xs font-semibold mb-2 uppercase tracking-wider" style={{ color: PRI2 }}>
                  I want to...
                </label>
                <div className="grid grid-cols-2 gap-3">
                  {[
                    { value: 'buyer',  icon: '🛒', label: 'Find Services', sub: 'Browse & book' },
                    { value: 'vendor', icon: '🏢', label: 'List Services', sub: 'Grow my business' },
                  ].map(opt => (
                    <button key={opt.value} type="button"
                      onClick={() => setFormData(p => ({ ...p, userType: opt.value }))}
                      className="p-3 rounded-xl text-left transition-all"
                      style={{
                        background: formData.userType === opt.value ? 'rgba(59,130,246,0.15)' : SURF2,
                        border: formData.userType === opt.value ? '2px solid #3b82f6' : '2px solid ' + BOR,
                      }}>
                      <div className="text-2xl mb-1">{opt.icon}</div>
                      <p className="text-xs font-bold" style={{ color: formData.userType === opt.value ? PRI2 : T1 }}>{opt.label}</p>
                      <p className="text-[10px]" style={{ color: T3 }}>{opt.sub}</p>
                    </button>
                  ))}
                </div>
              </div>
            ...
            <>
              <div className="relative my-5">
                ...
                <div className="relative flex justify-center">
                  <span className="px-3 text-xs" style={{ background: SURF, color: T3 }}>Already have an account?</span>
                </div>
              </div>
        ...
        <div className="flex items-center justify-center gap-4 mt-6">
          <div className="flex items-center gap-1.5 text-xs" style={{ color: T3 }}>
            <Shield className="w-3.5 h-3.5" style={{ color: ACC }} /> Secure & Private
          </div>
          <div className="flex items-center gap-1.5 text-xs" style={{ color: T3 }}>
